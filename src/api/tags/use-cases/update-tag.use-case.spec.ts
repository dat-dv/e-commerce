...
describe('UpdateTagUseCase', () => {
  ...
  it('should throw BadRequestException if tag not found', async () => {
    ...
    await expect(useCase.execute('1', { name: 'New Name' })).rejects.toThrow(BadRequestException);
  });

  it('should update successfully', async () => {
    mockTagsRepository.findById.mockResolvedValue({ id: '1', name: 'Old Name' });
    mockTagsRepository.update.mockResolvedValue({ id: '1', name: 'New Name' });

    const result = await useCase.execute('1', { name: 'New Name' });

    expect(result).toEqual({ id: '1', name: 'New Name' });
    expect(mockTagsRepository.update).toHaveBeenCalledWith('1', { name: 'New Name' });
  });
});
